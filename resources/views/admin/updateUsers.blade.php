                ...
                <body style="overflow: hidden visible;">
                    <div id="page-container" class="flex items-center justify-center h-screen">
                        <div class="bg-white w-full md:w-1/2 p-8 shadow-lg rounded-md">
                            <h1 class="text-3xl font-bold mb-6">Update User</h1>
                            <form action="{{route('updateUserDATA',$userDATA->id)}}" method="get">
                                <div class="mb-4">
                                    <input hidden type="hidden" id="Id" name="Id"
                                        value="{{ $userDATA->id }}">
                                </div>
                                ...
                                <div class="mb-4">
                                    <label for="roles" class="text-sm font-semibold text-gray-600">Role</label>
                                    <select id="roles" name="roles" x-model="newCat"
                                        class="w-full px-4 py-2 border rounded-md mt-1 focus:outline-none focus:border-indigo-500"
                                        required>
                                        @foreach ($roles as $role)
                                            <option value="{{$role->id}}"
                                                {{ ($role->name == implode(', ', $userDATA->getRoleNames()->toArray())) ? 'selected' : '' }}
                                                >
                                                {{$role->name }}
                                            </option>
                                        @endforeach
